    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `PokemonFloor` (`id` INTEGER NOT NULL, `name` TEXT NOT NULL, `imageUrl` TEXT NOT NULL, `weight` INTEGER NOT NULL, `baseExperience` INTEGER NOT NULL, PRIMARY KEY (`id`))');
...
class _$PokemonDao extends PokemonDao {
  ...
  Future<List<PokemonFloor>> findPokemonPage(int pageSize, int offset) async {
    return _queryAdapter.queryList(
        'SELECT * FROM PokemonFloor limit ?1 offset ?2',
        mapper: (Map<String, Object?> row) => PokemonFloor(
            row['id'] as int,
            row['name'] as String,
